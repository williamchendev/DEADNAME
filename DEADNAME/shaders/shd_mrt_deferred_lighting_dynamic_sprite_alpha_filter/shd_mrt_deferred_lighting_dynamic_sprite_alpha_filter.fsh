//
// (Multi Render Target) Dynamic Sprite with Alpha Filtering fragment shader for Inno's Deferred Lighting System
//
...
// Uniform Normal, MetallicRoughness, Emissive Map, and Alpha Filter Map Textures
uniform sampler2D gm_NormalTexture;
uniform sampler2D gm_MetallicRoughnessTexture;
uniform sampler2D gm_EmissiveTexture;
uniform sampler2D gm_AlphaFilterTexture;

// Fragment Shader
void main()
{
	// Diffuse Map & Alpha Filter Effect
	vec4 Diffuse = texture2D(gm_BaseTexture, v_vTexcoord_DiffuseMap);
	vec4 AlphaFilter = texture2D(gm_AlphaFilterTexture, v_vTexcoord_AlphaFilterMap);
	
	Diffuse.a = clamp(Diffuse.a - ((1.0 - AlphaFilter.r) * in_AlphaFilter), 0.0, 1.0);
    ...
    gl_FragData[1] = vec4(Normal.rgb, v_vColour.a * Normal.a);
