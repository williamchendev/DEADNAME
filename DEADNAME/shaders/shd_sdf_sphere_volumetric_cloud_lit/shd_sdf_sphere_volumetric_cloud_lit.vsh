...
void main() 
{
	...
	v_vWorldPosition = u_PlanetPosition + cloud_planet_center_offset + planet_position_horizontal + planet_position_vertical;
	
	// Interpolated Sample Position
	vec3 sample_position_horizontal = camera_right * in_Position.x * u_vsh_CloudSampleRadius * inverse_planet_rotation_matrix;
	vec3 sample_position_vertical = camera_up * in_Position.y * u_vsh_CloudSampleRadius * inverse_planet_rotation_matrix;
	...
	vec4 object_space_pos = vec4(cloud_vertex_position.xyz * inverse_vertical_vector + vec3(in_vsh_CameraDimensions * 0.5, 0.0), 1.0);
