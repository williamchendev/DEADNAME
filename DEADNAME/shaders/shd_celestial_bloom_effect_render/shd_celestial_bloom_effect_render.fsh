//
// Surface Bloom Effect fragment shader meant for Inno's Solar System Overworld
//
...
void main() 
{
	...
	for (float x = -BloomBlurSize; x <= BloomBlurSize; x++)
	{
		for (float y = -BloomBlurSize; y <= BloomBlurSize; y++)
		{
			...
			float Neighbor = texture2D(in_EmissiveMap, v_vTexcoord + in_TexelSize * vec2(x, y)).r * v_vColour.a;
