...
uniform vec3 in_camera_dimensions;

// Planet Properties
uniform float f_Elevation;
...
void main() 
{
	...
	vec3 view_direction = normalize(in_camera_position - v_vPosition);
	...
	//
	float ocean_elevation = u_Ocean_Elevation * (f_Elevation * 1.0);
	
	// Sphere UV Calculation
	vec2 sphere_uv = vec2(0.5 - atan2(-v_vTexVector.x, -v_vTexVector.z) / (2.0 * Pi), 0.5 - asin(-v_vTexVector.y) / Pi);
	
	// Calculate Ocean Foam Difference from Heightmap Texture Height Value at Sphere UV
	float elevation_height = texture2D(in_heightmap_texture, sphere_uv).r;
	float ocean_foam_height = (u_Ocean_Foam_Size / f_Elevation);
	float ocean_foam_difference = elevation_height - u_Ocean_Elevation - 0.1;
	
	// Establish Diffuse Texture Color at Sphere UV
	vec4 diffuse_color = u_Ocean_Color;
