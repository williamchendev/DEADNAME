...
// Check to Render either Celestial Simulator's UI or Default Platformer's UI
if (CelestialSimulator.active)
{
	// Establish Celestial Simulator Default Cursor Condition Variables
	var temp_celestial_grab_cursor = false;
	
	// Check if Celestial Simulator is Observing a Celestial Object and Dragging the Camera around said Celestial Object
	if (instance_exists(CelestialSimulator.camera_observing_instance) and CelestialSimulator.camera_observing_drag)
	{
		temp_celestial_grab_cursor = true;
	}
	
	// Cursor UI Condition Tree Behaviour
	if (temp_celestial_grab_cursor)
	{
		// Draw Grab Action Cursor
		draw_sprite(sUI_CursorGrab, 0, cursor_x, cursor_y);
	}
	else
	{
