...
repeat (array_length(solar_systems))
{
	...
	// Iterate through all the Celestial Bodies within the given Solar System
	var temp_celestial_body_index = 0;
	
	repeat (array_length(temp_solar_system))
	{
		// Find the given Celestial Body at the given Celestial Body Index within the Solar System
		var temp_celestial_body = temp_solar_system[temp_celestial_body_index];
		
		// Perform Celestial Body Simulation
		with (temp_celestial_body)
		{
			// Update Orbital Rotation around Solar System's Origin
			orbit_angle += orbit_speed * frame_delta;
			orbit_angle = orbit_angle mod 360;
			
			// Update Celestial Body's Rotation around Y Axis
		...
		// Decrement the Celestial Body Index
