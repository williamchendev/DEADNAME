		...
		if (editor_object.edges_calc) {
			...
			*/
			...
			if (editor_object.object_show_details) {
				...
				// Draw Details
				draw_text_outline(x + temp_bbox_right, y + temp_bbox_top, c_white, c_black, temp_detail_id);
				draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 9, c_white, c_black, temp_detail_type);
				draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 18, c_white, c_black, temp_detail_name);
				draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 27, c_white, c_black, temp_detail_pos);
			}
			else {
				draw_text_outline(x + temp_bbox_right, y + temp_bbox_top, c_white, c_black, object_get_name(global.editor_data[editor_object.object_editor_id, 0]));
				if (editor_object.mouse_press) {
					if (editor_object.mouse_mode == 0) {
						...
						draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 9, c_white, c_black, "Angle: " + string(temp_draw_angle) + "°");
					}
					else if (editor_object.mouse_mode <= 2) {
						draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 9, c_white, c_black, "X Scale: " + string(editor_object.object_x_scale));
						draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 18, c_white, c_black, "Y Scale: " + string(editor_object.object_y_scale));
					}
					else if (editor_object.mouse_mode == 3) {
						draw_text_outline(x + temp_bbox_right, y + temp_bbox_top + 9, c_white, c_black, "[X = " + string(round(x)) + ", Y = " + string(round(y)) + "]");
					}
				}
			}
