...
for (var q = 0; q < temp_blocks; q++) {
	...
	var temp_new_tileset = tileset_create(temp_level_x, temp_origin * 48, temp_width, temp_height, layer_get_id("Tiles"), temp_tileset_index, c_white, 1);
	var temp_tiledata_tileset = ds_list_find_value(temp_block_tiledata, q);
	for (var h = 0; h < temp_height; h++) {
		for (var w = 0; w < temp_width; w++) {
			tileset_set(temp_new_tileset, w, h, temp_tiledata_tileset[w, h]);
		}
	}
