...
/// @description Creates vertices for a geodesic icosphere using strip-based generation
/// @param {real} resolution Resolution per strip (determines triangle density)
/// @return {struct} Struct containing vertices array and indices array

function geodesic_icosphere_create(resolution) 
{
	// Initialize Icosphere Strip Data
	var strips = [ create_strip(0), create_strip(1), create_strip(2), create_strip(3), create_strip(4) ];
	...
	var edge_rotation_angle = arccos(vec3_dot([0, 1, 0], get_corner(0, 1)));
	...
	repeat (iterations)
	{
		...
		var pos = quaternion_mul_vec3(quaternion_axis_angle(strip.bottom_right_axis, edge_rotation_angle * u / resolution), [0, -1, 0]);
		...
		repeat (resolution_v - 1)
		{
			...
			//
			var p_left = quaternion_mul_vec3(quaternion_axis_angle(left_axis, edge_rotation_angle * edge_angle_scale), left_start);
			var p_right = quaternion_mul_vec3(quaternion_axis_angle(right_axis, edge_rotation_angle * edge_angle_scale), right_start);
			
			//
			var axis = vec3_normalize(vec3_cross(p_right, p_left));
			var angle = arccos(vec3_dot(p_right, p_left)) * face_angle_scale;
			pos = quaternion_mul_vec3(quaternion_axis_angle(axis, angle), p_right);
...
function create_strip(id) 
{
    ...
    var low_left = get_corner(2 * id, -1);
    var low_right = get_corner(id == 4 ? 0 : 2 * id + 2, -1);
    var high_left = get_corner(id == 0 ? 9 : 2 * id - 1, 1);
    var high_right = get_corner(2 * id + 1, 1);
    ...
    return 
    {
        ...
        bottom_left_axis: vec3_normalize(vec3_cross(down_vec, low_left)),
        bottom_right_axis: vec3_normalize(vec3_cross(down_vec, low_right)),
        mid_left_axis: vec3_normalize(vec3_cross(low_left, high_left)),
        mid_center_axis: vec3_normalize(vec3_cross(low_left, high_right)),
        mid_right_axis: vec3_normalize(vec3_cross(low_right, high_right)),
        top_left_axis: vec3_normalize(vec3_cross(high_left, up_vec)),
        top_right_axis: vec3_normalize(vec3_cross(high_right, up_vec))
    };
}
...
function quaternion_axis_angle(axis, angle) 
{
...
function quaternion_mul_vec3(q, v) 
{
